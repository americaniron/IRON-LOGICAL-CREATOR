...
import { MessageSquare, Image, Video, Film, Speaker, Microphone, UploadCloud, BrainCircuit, XIcon, Crane } from './common/Icons';
import { useAppContext } from '../context/AppContext';
import { useLocalStorage } from '../hooks/useLocalStorage';

const Sidebar: React.FC = () => {
  const { activeTask, setActiveTask, isSidebarOpen, setIsSidebarOpen } = useAppContext();
  ...
  const getActiveColor = (id: string) => {
    if (id.startsWith('grok')) return 'bg-grok-magenta';
    if (id.startsWith('oai')) return 'bg-guest-green';
    return 'bg-heavy-yellow';
  }
  ...
  const getIconColor = (id: string) => {
      if (id.startsWith('grok')) return 'text-fuchsia-900';
      if (id.startsWith('oai')) return 'text-green-900';
      return 'text-cyber-cyan';
  }
  
  const getGlowStyle = (id: string) => {
      if (id.startsWith('grok')) return {boxShadow: '0 0 8px var(--grok-magenta)'};
      if (id.startsWith('oai')) return {boxShadow: '0 0 8px var(--guest-green)'};
      return {boxShadow: 'var(--hud-glow)'};
  }
        ...
        {items.map((item) => (
          <li key={item.id}>
            <button
              ...
              className={`w-full flex items-center p-3 rounded-sm transition-all duration-200 relative overflow-hidden group text-base md:text-lg ${
                activeTask === item.id
                  ? `${getActiveColor(item.id)} text-black shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]`
                  : 'text-gray-500 hover:text-white hover:bg-industrial-gray'
              }`}
            >
              <div className={`absolute top-0 left-0 bottom-0 w-1.5 ${getAccentColor(item.id)} transition-transform duration-300 scale-y-0 group-hover:scale-y-100 ${activeTask === item.id ? 'scale-y-100' : ''}`}
                style={getGlowStyle(item.id)}
              ></div>
              <div className={`ml-3 mr-4 transition-transform group-hover:scale-110 ${activeTask === item.id ? `scale-110 ${getIconColor(item.id)}` : ''}`}>
                {item.icon}
              </div>
  ...
  return (
    <nav className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-[#1F2328] to-[#111317] border-r-2 border-black flex flex-col shadow-2xl transform transition-transform duration-300 ease-in-out md:relative md:w-80 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="absolute right-0 top-0 bottom-0 w-1 bg-gradient-to-b from-transparent via-[#EBB700] to-transparent opacity-20"></div>
      ...
      <div className="p-4 sm:p-6 flex flex-col items-center border-b-2 border-black bg-black/30 shadow-inner">
        ...
        <div 
          ...
          className="w-full max-w-[200px] h-32 mb-4 border-2 border-dashed border-[#333840] hover:border-[#EBB700] transition-colors cursor-pointer flex flex-col items-center justify-center bg-[#111317] relative group overflow-hidden"
        >
          {customLogo ? (
            <>
              <img 
              ...
              <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                <p className="font-['Black_Ops_One'] text-sm font-bold text-[#EBB700] uppercase tracking-widest">REPLACE</p>
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center text-gray-600 group-hover:text-[#EBB700] transition-colors">
        ...
        <div className="text-center american-iron-font">
          <p className="text-xs font-black text-[#EBB700] tracking-[0.4em] uppercase mb-1">IRON</p>
          <h1 className="text-5xl md:text-6xl font-black text-white tracking-tighter">MEDIA</h1>
          <p className="text-[9px] font-mono text-gray-500 tracking-widest mt-3 uppercase border-t border-[#333] pt-2">SITE_ORCHESTRATOR</p>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto mt-4 space-y-6 scrollbar-thin">
        ...
        <div>
          <h2 className="px-4 pb-2 text-xs font-mono text-gray-600 uppercase tracking-[0.3em] border-b border-industrial-gray flex items-center gap-2">
            <BrainCircuit className="h-4 w-4 text-guest-green" />
        ...
        <div>
          <h2 className="px-4 pb-2 text-xs font-mono text-gray-600 uppercase tracking-[0.3em] border-b border-industrial-gray flex items-center gap-2">
            <XIcon className="h-4 w-4 text-grok-magenta" />
            X-Corp Systems (Grok)
          </h2>
          <div className="mt-2">
      ...
      <div className="p-4 border-t-2 border-black bg-black/30">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs font-mono text-gray-400 uppercase tracking-widest">SYSTEM: <span className="text-green-400">NOMINAL</span></p>
        </div>
      </div>
    </nav>
  );
